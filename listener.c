...
void start_wavfile(SNDFILE ** sndfile_out, char *fname, SF_INFO * pars)
{
    ...
    if (fname_template) {
        ...
        sprintf(fname, "%s/%04d-%02d-%02d_%02d:%02d:%02d.wav",
                ...
                tmdummy->tm_mon + 1, tmdummy->tm_mday, tmdummy->tm_hour, tmdummy->tm_min, tmdummy->tm_sec);
    }
    ...
    *sndfile_out = sf_open(fname, SFM_WRITE, pars);
    ...
    (void)sf_set_string(*sndfile_out, SF_STR_SOFTWARE,
                        "Generated with listener v" VERSION ", http://www.vanheusden.com/listener/");
...
void start_wav_file_processor(char *fname)
{
    if (exec) {
        ...
        if (!silent)
            printf("Starting childprocess: %s\n", exec);
...
int start_piped_proc(char *what)
{
    ...
    if (pid == 0) {
        ...
        if (-1 == execlp("/bin/sh", "/bin/sh", "-c", what, (void *)NULL))
            error_exit("execlp of %s failed\n", what);
        exit(1);
    }
...
void record(PaStream * pcm_handle, int cur_buffer, SF_INFO * file_pars)
{
    ...
    if (!save_after_filter)
        temp_buffer = (short *)mymalloc(2 * buffer_size, "recording buffer(3)");
    ...
    if (!silent)
        printf("Writing pre-sound data to disk\n");
    ...
    for (; do_exit == 0;) {
        ...
        if (output_pipe) {
            ...
            if ((n_wr =
                 sf_write_short(sndfile_out, save_after_filter ? buffer : temp_buffer,
        ...
        if (threshold_count < min_triggers && (now_ts - start_of_recording_ts) > min_duration
            && (now_ts - sound_detected_ts) > rec_silence) {
            if (!silent)
                printf("Stopped recording: %d triggers\n", threshold_count);
            /*  then exit */
            break;
        }
        ...
        if (max_duration != -1 && (now_ts - start_of_wavfile_ts) > max_duration) {
            if (!silent)
                printf("Splitting up audio-file\n");
    ...
    if (!silent)
        printf("Stopped recording (%d seconds)\n", (int)(now_ts - start_of_recording_ts));
}
...
int main(int argc, char *argv[])
{
    ...
    for (;;) {
        ...
        if (strcasecmp(cmd, "wav_path") == 0) {
            ...
            if (strcasecmp(par, "yes") == 0 ||
                strcasecmp(par, "on") == 0 || strcasecmp(par, "true") == 0 || strcasecmp(par, "1") == 0) {
                save_after_filter = 1;
            } else {
                save_after_filter = 0;
            }
    ...
    if (silent == 0) {
        ...
        if (amplify) {
            if (fixed_ampl_fact)
                printf("Using fixed amplification\n");
            printf("start amplify: %f\n", start_amplify);
            if (!fixed_ampl_fact)
                printf("Max. amplify:  %f\n", max_amplify);
        }
        if (from_pipe)
    ...
    /* initialize filters */
    for (loop = 0; loop < n_filter_lib; loop++) {
        filter[loop].lib_data =
            filter[loop].init_library(sizeof(short), audio_pars->channels, audio_pars->samplerate, filter[loop].par);
    }
    ...
    for (; do_exit == 0;) {
        ...
        if (!pr_buffers[cur_buffer]) {
            pr_buffers[cur_buffer] =
                (short *)mymalloc(2 * audio_pars->samplerate * audio_pars->channels * sizeof(short),
                                  "recording buffer(1)");
        }
